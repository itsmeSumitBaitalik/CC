  ...
  return (
    <>
      ...
      <div className={`px-4 pt-4 pb-2 flex-shrink-0 flex items-center justify-between ${isSidebarCollapsed ? "md:justify-center" : ""}`}>
        ...
        {/* Mobile Close Toggle */}
        <button 
          className="md:hidden flex w-7 h-7 bg-white border-2 border-black shadow-retro-sm items-center justify-center hover:bg-black hover:text-white transition-colors flex-shrink-0 cursor-pointer"
          onClick={() => setIsSidebarOpen(false)}
        >
          <span className="material-symbols-outlined text-sm font-black">keyboard_double_arrow_left</span>
        </button>

        {/* Desktop Collapse Toggle */}
        <button 
          className="hidden md:flex w-7 h-7 bg-white border-2 border-black shadow-retro-sm items-center justify-center hover:bg-black hover:text-white transition-colors flex-shrink-0 cursor-pointer"
        ...
        {navItems.map((item) => {
          ...
          if (item.to === "/dashboard/chat") {
            return (
              <a
                ...
                className={`flex items-center gap-3 py-3 border-3 border-transparent text-black hover:border-black hover:bg-retro-yellow hover:shadow-retro-sm transition-all px-3 ${isSidebarCollapsed ? "md:justify-center md:px-0" : ""}`}
              >
          ...
          return (
            <NavLink
              ...
              onClick={() => setIsSidebarOpen(false)} // close modal on mobile upon click
            >
              <div className={`nav-icon relative w-9 h-9 flex items-center justify-center flex-shrink-0 ${item.iconBg}`}>
                <span className={`material-symbols-outlined ${item.iconColor} text-xl`}>{item.icon}</span>
                {item.badge && (
        ...
        <div className={`bg-retro-yellow border-3 border-black p-3 shadow-retro flex-1 ${isSidebarCollapsed ? "md:hidden" : ""}`}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="retro-title text-sm leading-none truncate">Jackie Chen</p>
                <p className="retro-subtitle mt-0.5 truncate">2nd Year • CS</p>
              </div>
              <button
                onClick={() => setShowSettings(true)}
