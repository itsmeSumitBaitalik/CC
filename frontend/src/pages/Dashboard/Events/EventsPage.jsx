...
const mapEventCard = (ev) => {
  ...
  return {
    ...
    seatLabel: ev.totalSeats ? `${ev.registeredCount}% seats filled` : null,
    registered: false,
...
export default function EventsPage() {
  ...
  useTopbar({ subtitle: "Campus Life", title: "Events 🎉" });
  ...
  const dynamicStats = useMemo(() => {
    return [
      { icon: 'upcoming', count: String(upcomingEvents.length), label: 'Upcoming', bg: 'bg-retro-green' },
      { icon: 'play_circle', count: String(ongoingEvents.length), label: 'Ongoing', bg: 'bg-retro-red' },
      { icon: 'check_circle', count: String(completedEvents.length), label: 'Completed', bg: 'bg-white', textClass: 'text-black' },
      { icon: 'bookmark', count: String(registeredIds.size), label: 'Registered', bg: 'bg-retro-yellow', textClass: 'text-black' },
    ];
